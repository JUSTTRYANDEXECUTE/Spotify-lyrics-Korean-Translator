...
- 일본 노래의 경우: 한자 표기와 실제 발음(読み)이 다른 "当て字" 기법을 인식할 것
  예) "運命共同体"라고 쓰고 "しんゆう(親友)"로 읽으면 "절친"으로 번역
  예) "世界"라고 쓰고 "きみ"로 읽으면 "너"로 번역
- 연애/사랑 노래 맥락 고려: 
  예) "お試し期間" → "시험 기간" (시험 삼아 사귀는 기간)
  예) "告白" → "고백"
- 한국에서 자연스럽게 쓰이는 표현으로 의역
